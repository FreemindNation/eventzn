      ...
      {loading ? (
        ...
        <div className="space-y-4">
          {events.map((event) => (
            <div
              key={event.id}
              className="flex-col sm:flex-row p-4 shadow-lg rounded-lg bg-white dark:bg-gray-900 transition-all duration-300 border border-transparent hover:border-purple-500 hover:shadow-purple-500/50 flex items-center gap-4"
            >
              {event.imageUrl && (
                <Image
                  alt={event.title}
              ...
              <Button as="a" color="primary" href={`/events/${event.id}`} variant="light">
                View Event
              </Button>
            </div>
          ))}
